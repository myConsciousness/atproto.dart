...
void main() {
  group('createMockedSubscription (P-11)', () {
    ...
    test(
      'close() always delivers a done event (no hang)',
      () async {
        final mocked = createMockedSubscription<Map<String, dynamic>>();
    ...
    test(
      'close() delivers done even without a listener attached',
      () async {
        final mocked = createMockedSubscription<Map<String, dynamic>>();
    ...
    test(
      'connection errors propagate to the stream then done',
      () async {
        final mocked = createMockedSubscription<Map<String, dynamic>>();
        final errors = <Object>[];
        final done = Completer<void>();
    ...
    test(
      'server-side close tears down the stream',
      () async {
        final mocked = createMockedSubscription<Map<String, dynamic>>();
        final done = Completer<void>();
        mocked.subscription.stream.listen((_) {}, onDone: done.complete);
