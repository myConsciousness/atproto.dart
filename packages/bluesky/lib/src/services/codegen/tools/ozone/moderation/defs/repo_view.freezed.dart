...
class _RepoView implements RepoView {
  const _RepoView({this.$type = 'tools.ozone.moderation.defs#repoView', required this.did, required this.handle, this.email, required  List<Map<String, dynamic>> relatedRecords, @JsonKey(toJson: iso8601) required this.indexedAt, @ModerationConverter() required this.moderation, @InviteCodeConverter() this.invitedBy, this.invitesDisabled, this.inviteNote, @JsonKey(toJson: iso8601) this.deactivatedAt, @ThreatSignatureConverter()  List<ThreatSignature>? threatSignatures,  Map<String, dynamic>? $unknown}): _relatedRecords = relatedRecords,_threatSignatures = threatSignatures,_$unknown = $unknown;
