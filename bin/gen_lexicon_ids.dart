...
void main() {
  ...
  for (final entry in packages.entries) {
    ...
    File(
      './packages/$package/lib/src/$_idsFileName',
    ).writeAsStringSync(ids.toString());

    File(
      './packages/$package/lib/src/$_nsidsFileName',
    ...
    for (final lexiconPath in directory.listSync()) {
      for (final lexicon in Directory(lexiconPath.path).listSync()) {
        ...
        defs.forEach((key, _) {
          if (key != 'main') {
            fields.add(
              Field('$fieldName${_toFirstUpperCase(key)}', '$id#$key'),
            );
          }
        });
      }
...
String _toFieldName(final String id) => id
    .split('.')
